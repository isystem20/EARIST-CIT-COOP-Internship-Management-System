...
class UserController extends MY_Controller {
	...
	public function LoadUserMasterlist($filter = null,$value = null) {
		$layout = array('datatable'=>TRUE, 'page_title'=>'Student Masterlist');
		if (!empty($filter)) {
				$data['all_list_stud'] = $this->acct->LoadAccounts($filter,$value,'STUDENT');
				$data['all_list_admin'] = $this->acct->LoadAccounts($filter,$value,'ADMIN');	
		}else {
			$data['all_list_stud'] = $this->acct->LoadAccounts(null,null,'STUDENT');	
			$data['all_list_admin'] = $this->acct->LoadAccounts(null,null,'ADMIN');			
		}
	...
	public function RegisterStudent(){ 
		$layout = array('form'=>TRUE,'formelements'=>TRUE, 'page_title'=>'User Registration');
		...
		$this->load->view('layout/topbar');
		$this->load->view('layout/leftbar',$l);
		$this->load->view('pages/adminregister',$data);
		// $this->load->view('layout/rightbar');
		$this->load->view('layout/scripts',$layout);
		$json = json_encode($data); //log
	...
	public function ViewStudent($id){ 
		$layout = array('form'=>TRUE,'formelements'=>TRUE, 'page_title'=>'User Details');
		$data['cities'] = $this->city->LoadMasterlist('Id,Name');
		$data['regions'] = $this->region->LoadMasterlist('Id,Name');
		$data['courses'] = $this->course->LoadMasterlist('Id,Name');
		$data['semesters'] = $this->semester->LoadMasterlist('Id,Name');
		...
		$this->load->view('layout/topbar');
		$this->load->view('layout/leftbar',$l);
		$this->load->view('pages/loadadmin',$data);
		// $this->load->view('layout/rightbar');
		$this->load->view('layout/scripts',$layout);
		$json = json_encode($data); //log
	...
	public function UpdateStudent($id){ 
		$layout = array('form'=>TRUE,'formelements'=>TRUE, 'page_title'=>'User Update');
		$data['cities'] = $this->city->LoadMasterlist('Id,Name');
		$data['regions'] = $this->region->LoadMasterlist('Id,Name');
		$data['courses'] = $this->course->LoadMasterlist('Id,Name');
		$data['semesters'] = $this->semester->LoadMasterlist('Id,Name');
	...
	public function Create() {
        ...
        $this->form_validation->set_rules('Code', 'Employee Number', 'required|is_unique[tbl_students.Code]');
        ...
        if ($this->form_validation->run() == FALSE){
    		...
    		if ($senderror == TRUE) {
		        ...
		        $result = $this->students->AddStudent($postdata,'ADMIN');
	     		if ($result != FALSE) {	
					$json = json_encode($result); //log
			        $this->logger->log('Success Register','Studdents',$json); //Log 
			        if (!empty($this->session->userdata('userid'))) {
			        	echo json_encode(['redirect'=>base_url('manage/accounts')]);
			        }
			        else {
	...
	public function Delete() {
		...
		if (empty($postdata['Id'])) {
			$json = json_encode($postdata); //log
	        $this->logger->log('Invalid Register','Students',$json); //Log  
            echo json_encode(['error'=>'Unknown Record']);
		}else{
			$id = $postdata['Id'];
			unset($postdata['Id']);
